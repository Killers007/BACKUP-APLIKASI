...
class Excel extends PHPExcel {
    ...
    public function generateExcel($data,$file,$field,$startCol = "A")
    {
            ...
            foreach (range('A', 'J') as $value) {
                $acSheet->getStyle("{$value}:{$value}")->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_TEXT);
            }
            ...
            foreach ($data as $val) {
                ...
                foreach($field as $f=>$t){
                    if(strpos(strtoupper($f),"NIM") !== false || strpos(strtoupper($f),"NIU") !==  false){
                        $acSheet->setCellValueExplicit("$c$i", $val[$f],PHPExcel_Cell_DataType::TYPE_STRING);
                    }else{
