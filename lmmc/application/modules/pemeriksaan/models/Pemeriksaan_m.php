 ...
 */
class Pemeriksaan_m extends MY_Model
{
    ...
    public function val_update_hasil()
    {
        ...
        $this->form_validation->set_rules('dokter_jaga', 'Dokter Jaga', 'required|trim');
        ...
        if ($this->form_validation->run() == FALSE) {
            $validasi = [
                ...
                'dokter_jaga' => form_error('dokter_jaga'),
            ...
            $data = [
                ...
                'hasilNipPemeriksa' => $this->session->userdata('user')['username'],
                'hasilJagaId' => $this->input->post('dokter_jaga'),
        ...
        if (count($dt) == 2) {
            ...
            if ($c_pes->num_rows() > 0) {
                ...
                if ($c_kli > 0 && $c_pes->num_rows() > 0 && $validasi_klinik == 1) {
                    ...
                    $this->db->join('lmmc_t_jagaklinik', 'jagaId = hasilJagaId', 'left');
    ...
    public function get_klinik_dokter($klinikId)
    {
        $jalur = $this->getJalurActive();

        $this->db->where('jagaKlinikid', $klinikId);
        $this->db->where('jagaJalurid', @$jalur->jalurId);
        $this->db->join('lmmc_m_dokter', 'dokterNip = jagaDokterid');
        $data = $this->db->get('lmmc_t_jagaklinik')->result();

        $res = [];
        foreach ($data as $key => $value) {
            $res[$value->jagaId] = $value->dokterNama;
        }
    ...
    public function pesertaTemplate($data)
    {
        ...
        <header class="panel-heading bg-light no-border mar-bot" style="background-color:#fff">
            <div class="col-md-12 mar-bot">
                ...
                <a href="#" class="pull-left thumb-md avatar b-3x m-r">
                    <img src="' . base_url('pemeriksaan/image/path_foto_peserta/');
        if ($data->pesertaFoto) {
        ...
        $html .= '
            <div class="col-md-12 mar-bot">
                <label class="col-sm-3 control-label">Dokter Jaga</label>
                <b>
                    <div class="col-sm-9">';

        $jagaId = (!$data->hasilJagaId) ? $this->session->dokterJaga : $data->hasilJagaId;
        $dokter = form_dropdown('dokter_jaga', $this->get_klinik_dokter($data->klinikId), $jagaId, array('class' => 'form-control', 'style' => 'width:100%'));
        $html .= $dokter;

        $html .= '<div id="dokter_jaga-v" name="inVal" style="margin-bottom:0px !important;"></div>
                    </div>
                </b>
            </div>
        ';
        ...
        <div class="col-md-12" > 
            <div class="btn-group btn-group-justified" style="padding: 0 15px;"> 
                <a href="#" class="btn btn-success" onclick="updHasil()" style="background-color:#1BB399 !important;border-color:#1BB399 !important;">
                    <span class="fa fa-save"></span>&nbsp    
                    <b>Simpan</b>
                </a>
            </div> 
        </div>
