            ...
            <header class="header bg-white b-b b-light">
                <section class="row m-b-md" style="">
                    <div class="col-xs-9">
                        ...
                        ?>
                        <h1 class="m-b-xs text-black hidden-xs"><small><span class="text-black">Jaga Klinik |</span> <span style="font-size: 13px"><?php echo $text; ?></span></small></h1>
                        <h2 class="m-b-xs text-black hidden-lg hidden-md hidden-sm"><small></small></h2>
                    </div>
                    <div class="col-xs-3 m-t-lg pull-right">
                       <a  onclick="setTitle('Tambah', 'Tambah')" class="btnShowModal btn btn-primary pull-right" href="<?php echo base_url('jaga_klinik') ?>/tambah" style="margin-right:5px"><i class="fa  fa-plus"></i> <span class="hidden-xs">Tambah Data</span></a>
                       <br>
                ...
                <table id="table" width="100%" class="table table-hover table-bordered" style="margin-top: -500px"  cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style="vertical-align: middle">Nama Klinik</th>
                            <th style="vertical-align: middle" width=15%>Tanggal</th>
                            <th style="vertical-align: middle">Dokter Jaga</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        function _initialDatatable()
        {
            ...
            columnDefs: [{"className": "dt-tengah", "targets": [0,4]}],
            order: [[0, "asc"], [1, "desc"]],
            'searching'   : true,
            pagingType: 'numbers',
            language:{
            ...
            columns: [
            {
                data: 'jagaId', render: function (data, type, row, meta) {
                    return meta.row + meta.settings._iDisplayStart + 1;
                }
            },
            {data: 'klinikNama'},
            {data: 'jagaTanggal'},
            {data: 'listDokter', searchable: false, orderable: false},
            {
                data: null, searchable: false, orderable: false, render: function (data) {
                   
                    var btnUbah =  `&nbsp;<a href="<?php echo base_url('jaga_klinik') ?>/ubah/`+data.jagaId+`" data-id="`+data.jagaId+`" class="btn btn-xs btn-default" title="View">
                    <i style="color: #1aabbb;"class="i i-pencil2"></i>
                    </a>` ;

                    var btnHapus = `<a class="btn btn-xs btn-default btnDelete" data-toggle="tooltip" data-id='`+data.jagaId+`' data-nama='`+data.jagaId+`' data-backdrop='static' data-toggle='modal' data-target='#modal-hapus' title='Hapus'><span style='color : #e33244' class='i i-trashcan'></span></a> `;
                    
                    var btnDetail = `&nbsp;<a class="btn btn-xs btn-default btnDetail" data-toggle="tooltip" data-toggle='modal' data-id='`+data.jagaId+`' data-target='#modal-detail' title='Detail'><span style='color : #177bbb' class='fa fa-search'></span></a> `;
