            ...
            <header class="header bg-white b-b b-light">
                <section class="row m-b-md" style="">
                    <div class="col-xs-9">
                        ...
                        $text = '';
                        ?>
                        <h1 class="m-b-xs text-black hidden-xs"> <small class="m-b-xs text-black">Dokter <span style="font-size: 13px"><?php echo ucwords(strtolower($text)); ?></span></small></h1>
...
<div class="modal fade" id="modal-reset" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                <h4 class="modal-title">Apakah anda yakin mereset password?</h4>
            </div>
            <div class="modal-body modalDeleteBody">
            ...
            <div class="modal-body">
                ...
                <table>
                    <tbody><tr>
                        <td>&nbsp;Password akan direset menjadi : <span class="dokterNip"></span></td>
                    </tr>
                </tbody>
            </table>
           </div>
            ...
            <div class="modal-footer" style="margin-right: -50px">
                <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-warning btnConfirmReset">Reset</button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        oTable = $('#table').dataTable({
            ...
            columns: [
            {
                data: 'dokterNip', render: function (data, type, row, meta) {
            ...
            {data: 'dokterNama'},
            {
                data: null, searchable: false, orderable: false, render: function (data) {
                    ...
                    var btnUbah =  `&nbsp;<a onclick="setTitle('Perbaharui', 'Perbaharui')" data-id="`+data.dokterNip+`" class="btnEdit btn btn-xs btn-default" title="View">
                    <i style="color: #1aabbb;"class="i i-pencil2"></i>
                    </a>` ;

                    var btnReset = `&nbsp;<a class="btn btn-xs btn-default btnDelete" data-toggle="tooltip" data-id='`+data.dokterNip+`' data-backdrop='static' data-toggle='modal' data-target='#modal-hapus' title='Hapus'><span style='color : #e33244' class='i i-trashcan'></span></a>`;

                    var btnHapus = `&nbsp;<a class="btn btn-xs btn-default btnReset" data-toggle="tooltip" data-id='`+data.dokterNip+`' data-backdrop='static' data-toggle='modal' data-target='#modal-reset' title='Reset'><span style='color : #fcc633' class=' fa fa-rotate-left '></span></a>`;
                    
                    var btnDetail = `&nbsp;<a class="btn btn-xs btn-default btnDetail" data-toggle="tooltip" data-toggle='modal' data-id='`+data.dokterNip+`' data-target='#modal-detail' title='Detail'><span style='color : #177bbb' class='fa fa-search'></span></a> `;

                        return btnReset + btnUbah + btnHapus;
                }
            },
            ],
        });
        ...
        $(document).on('click', '.btnEdit', function(event) {
            ...
            find = dataFull.find(seep => seep.dokterNip == id);
        ...
        function template(data)
        {
            ...
            var label = ['Nip Dokter', 'Nama Dokter'];
            var key   = ['dokterNip', 'dokterNama'];
            $('.dokterNip').html(data.dokterNip);
        ...
        $(document).on('click', '.btnConfirmDelete', function(event) {
         ...
         $(document).on('click', '.btnConfirmReset', function(event) {
            ...
            $.ajax({
                url: '<?php echo current_url() ?>/resetPassword/'+id,
                type: 'POST',
                dataType: 'JSON',
                beforeSend: function()
                {
