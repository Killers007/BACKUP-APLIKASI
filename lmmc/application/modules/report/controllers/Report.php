...
class Report extends MY_Controller {
	...
	function slice()
	{
		$array = array(
			array('tes' => 1, 'dua' => 'dua'),
			array('tes' => 4, 'dua' => 'duas'),
			array('tes' => 3, 'dua' => 'duass'),
			array('tes' => 5, 'dua' => 'duasss'),
			array('tes' => 6, 'dua' => 'duasss'),
		);
		...
		if ($this->input->is_ajax_request()) 
		{
			...
			else
			{
				...
				foreach ($res['data'] as $key => $value) {
					$res['data'][$key]->belumPeriksa = $value->jumPeserta - $value->sudahPeriksa;
				}
	...
	function report($_klinikId = null, $kategoriId, $_kodeProdi = null)
	{
		...
		$html = $this->load->view('report_baru/report4_v.php', $data, $isPdf);
	...
	function report_excel($_klinikId = null, $kategoriId, $_kodeProdi = null)
	{
		...
		{
			$_savePath = './'.@$datas[0]['jalur']->jalurNama.'_'.@$datas[0]['jalur']->jalurTahun.'_'.@$datas[0]['kategori'].'_'.@$datas[0]['klinik']->klinikNama.'_'.@$datas[0]['fakultas']->prodiNamaResmi.'.xlsx';
		}
		else
		{
		...
		foreach ($datas as $index => $data) {
			...
			$sheet1->setTitle('Prodi '.substr(@$data['fakultas']->prodiNamaResmi, 0, 24));
	...
	function report_jumlah_excel($_klinikId = null, $_kodeKategori = null)
	{
		...
		foreach ($data['prodi'] as $key => $value) 
		{
			$D = 324;
			$sheet1->setCellValue('A'.$_numStart, ($key + 1));
			$sheet1->setCellValue('B'.$_numStart, "{$value->fakNamaResmi} / {$value->prodiNamaResmi}");
