            ...
            <header class="header bg-white b-b b-light">
                <section class="row m-b-md" style="">
                    <div class="col-xs-9">
                        ...
                        $text = '';
                        ?>
                        <h1 class="m-b-xs text-black hidden-xs"><small><b>Report  </b><span style="font-size: 13px"><?php echo ucwords(strtolower($text)); ?></span></small></h1>
         ...
         <section class="scrollable padder space" > 
            <div class="box-body ">
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <?php echo form_dropdown('selectKlinik', [], '', array('class' => 'form-control chosen-select', 'style' => 'width:100%')); ?>
                        <div class="m-b"></div>
                    </div>
                    <div class="col-md-4">
                        <button class="btn btn-success" id="lihat" name="lihat"><i class="fa fa-eye"></i> Lihat</button>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#cetak', function(event) {
            window.open('<?php echo base_url('report/per_kategori/report_jumlah') ?>/'+klinikId+'/'+kategoriId, '_blank');
        });

        $(document).on('click', '#excel', function(event) {
            window.open('<?php echo base_url('report/per_kategori/report_jumlah_excel') ?>/'+klinikId+'/'+kategoriId, '_blank');
        });
        ...
        function _initialDatatable()
        {
            ...
            oTable = $('#table').dataTable({
                ...
                footerCallback: function ( row, data, start, end, display ) {
                    ...
                    if (monTotal != 0) 
                    {
                        var btnCetak = `&nbsp;<a target="_blank" href="<?php echo base_url('report/per_kategori/report') ?>/`+klinikId+`/`+kategoriId+`" class="btn btn-xs btnDetail" style="background-color: #1aabbb" data-toggle="tooltip" title='Cetak semua laporan klinik `+klinikNama+`'><span style='color : #FFFFFF' class=' fa fa-print'></span></a> `;
                        var btnExcel = `&nbsp;<a target="_blank" href="<?php echo base_url('report/per_kategori/report_excel') ?>/`+klinikId+`/`+kategoriId+`" class="btn btn-xs btnDetail" style="background-color: #1aabbb" data-toggle="tooltip" title='Cetak semua laporan klinik `+klinikNama+`'><span style='color : #FFFFFF' class='i i-file-excel'></span></a> `;
                ...
                {data: 'belumPeriksa', searchable: false, orderable: false},
                {
                    data: null, searchable: false, orderable: false, render: function (data) {
                        ...
                        var btnCetak = `&nbsp;<a target="_blank" href="<?php echo base_url('report/per_kategori/report') ?>/`+klinikId+`/`+kategoriId+`/`+data.prodiKode+`" class="btn btn-xs btn-default btnDetail" data-toggle="tooltip" title='Cetak'><span style='color : #1aabbb' class='fa fa-print'></span></a> `;
                        var btnExcel = `&nbsp;<a target="_blank" href="<?php echo base_url('report/per_kategori/report_excel') ?>/`+klinikId+`/`+kategoriId+`/`+data.prodiKode+`" class="btn btn-xs btn-default btnDetail" data-toggle="tooltip" title='Cetak'><span style='color : #1aabbb' class='i i-file-excel'></span></a> `;
        ...
        $(document).on('click', '.btnEdit', function(event) {
            ...
            find = dataFull.find(seep => seep.prodiKode == id);
